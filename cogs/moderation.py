    ...
    async def ban(self, ctx, member: str, args_first: str = "", args_second: str = ""):
        ...
        for member_i in member_array:
            ...
            if "<" in member_i and ">" in member_i and "@" in member_i:
                var_type = "mention"
                member_obj = await ctx.guild.fetch_member(int(self.mention_to_id(self, member_i)))
            ...
            if args_first == "" or not args_first.startswith('-'): #permanent
                ...
                embed.add_field(
                    name="Member banned",
                    value=member_obj.name + "#" + member_obj.discriminator + " has been banned `permanently`",
                    inline=False,
                )
